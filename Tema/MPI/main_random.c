/*
 * ============================================================
 * TEMA LABORATOR: Roy-Floyd-Warshall (MPI) - VARIANT RANDOM
 * ============================================================
 * Problema: Se da o matrice de adiacenta D de dimensiune NxN
 * reprezentand distantele dintr-un graf ALEATOR. Sa se gaseasca
 ...
 *
 * DIFERENTE: Matricea este generata ALEATOR cu densitate controlabila.
 *
 * Compilare: mpicc -o main_random main_random.c
 * Rulare:    mpirun -np 4 ./main_random
 * ============================================================
 */
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        printf("Matricea initiala a distantelor (random, INF=%d):\n", INF);
        printf("(Densitate: %d%%, Max distanta: %d)\n\n", DENSITY, MAX_DIST);
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                ...
                {
                    /* Generare aleatoare: daca random < DENSITY%, atunci exista muchie */
                    int rand_prob = rand() % 100;
                    if (rand_prob < DENSITY)
                    {
                        D[i * N + j] = (rand() % MAX_DIST) + 1; /* 1..MAX_DIST */
                    }
                    else
                    {
                        D[i * N + j] = INF;
